...
from torch.optim.lr_scheduler import (
    CosineAnnealingWarmRestarts, LambdaLR, CosineAnnealingLR, MultiStepLR,
    CyclicLR)
...
class LRScheduler:
    def __init__(self, c, name, optimizer):
        ...
        self.manual_schedulers = {'triangular', 'attention_is_all_you_need'}
        self.num_steps = 0

        if ((name not in self.manual_schedulers) and
                self.c.exp_optimizer != 'adafactor'):
    ...
    def construct_auto_scheduler(self):
        ...
        elif self.name == 'cosine_warmup':
            self.scheduler = get_cosine_schedule_with_warmup(
                optimizer=self.optimizer,
                num_warmup_steps=num_warmup_steps,
                num_training_steps=total_steps)
        elif self.name == 'cosine_warmup_hard_restarts':
            self.scheduler = (
                get_cosine_with_hard_restarts_schedule_with_warmup(
                    optimizer=self.optimizer,
                    num_warmup_steps=num_warmup_steps,
                    num_training_steps=total_steps,
                    num_cycles=1))  # Default 1: number of hard restarts to use
        elif self.name == 'cosine_warmup_3_hard_restarts':
            self.scheduler = (
                get_cosine_with_hard_restarts_schedule_with_warmup(
                    optimizer=self.optimizer,
                    num_warmup_steps=num_warmup_steps,
                    num_training_steps=total_steps,
                    num_cycles=3))  # Default 1: number of hard restarts to use
        elif self.name == 'cosine_warm_restarts':
            self.scheduler = CosineAnnealingWarmRestarts(
                optimizer=self.optimizer,
                T_0=int(num_warmup_steps),
                # Increases number of steps between each restart
        ...
        elif self.name == 'multistep':
            if self.c.exp_multistep_milestones is not None:
                milestones = self.c.exp_multistep_milestones
            else:
                milestones = [
                    int(total_steps * 0.4),
                    int(total_steps * 0.6),
    ...
    def step(self):
        ...
        if self.c.exp_optimizer == 'adafactor':
            pass
        elif self.name == 'triangular':
            if (scale := num / (0.2 * tot)) <= 1:
                self.optimizer.param_groups[0]['lr'] = c_lr * scale
            elif (scale := num / tot) >= 0.8:
                # scale starts at 0.8 and ends at 1
                self.optimizer.param_groups[0]['lr'] = (
                        c_lr * ((0.8 - scale) / 0.2 + 1))
            else:
                pass
        elif self.name == 'attention_is_all_you_need':
            # Only use 2000 steps to reflect the fact that we don't train
            # our models for 300,0000 steps like they do.
            self.optimizer.param_groups[0]['lr'] = (
                    self.c.model_dim_hidden ** (-0.5) *
                    min(self.num_steps ** (-0.5),
                        self.num_steps * 2000 ** (-1.5)))
        elif self.name == 'cosine_cyclic':

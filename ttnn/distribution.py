...
from ttnn.column_encoding_dataset import TTNNDataset
from ttnn.train import Trainer
from ttnn.utils.model_init_utils import (
...
def distributed_train_wrapper(gpu, args):
    ...
    supcon_target_col = args['supcon_target_col']
    model, optimizer, scaler = init_model_opt_scaler_from_dataset(
        dataset=dataset, c=c, device=gpu, supcon_target_col=supcon_target_col)
    model = setup_ddp_model(model=model, c=c, device=gpu)

    distributed_dataset = TTNNDataset(dataset)
    dist_args = {
        'world_size': world_size,
        'rank': rank,
        'gpu': gpu}

...
class HPC_layer(nn.Module):
    def __init__(self, layer, latent_vector_size, dim_emb, num_groups):
        super().__init__() 
        self.layer = layer
    ...
    def forward(self, x):
        ...
        return torch.cat([self.layer([self.ind[i].repeat(batches[i].size(0), 1, 1), batches[i]]) for i in range(len(batches))], dim=1)  


class Hierarchical_Perceiver(nn.Module):
    '''this model specifies a hierarchical perceiver that does not make use of weight sharing in each of the groups'''
    def __init__(self, dim_in, c):
        super().__init__()
        print('running fixed (?) perceiver')
        if c.model_type == 'h-npt-large':
            self.HPC = large_HPC
        else:
            raise NotImplementedError

        self.HPC['input_channels'] = [dim_in] + self.HPC['latent_channels'][:-1]
        self.layers = nn.ModuleList([])
        self.final_attention = Attention(query_dim=dim_in, context_dim=self.HPC['latent_channels'][-1], output_dim=self.HPC['latent_channels'][-1])
        ...
        for index, layer in enumerate(self.HPC['encode']):
            ...
            layer_group.append(get_cross_attn())    
            layer_group += [get_cross_ff() for _ in range(num_ff)]

            for _ in range(self.HPC['self-attention_layers'][index]):
                layer_group.append(get_self_attn()) 
                layer_group += [get_self_ff() for _ in range(num_ff)]

            layer_group = nn.Sequential(*layer_group)
        
            self.layers.append(HPC_layer(layer_group, self.HPC['latent_vectors_per_group'][index], self.HPC['latent_channels'][index], self.HPC['groups'][index]))    

    def forward(self, x):
        new_x = x
        cache = {}
        for i, layer in enumerate(self.layers):
            if self.HPC['decode'][i] != None:
                new_x = layer(new_x + cache[self.HPC['decode'][i]])
            else:
                new_x = layer(new_x)
            if self.HPC['encode'][i] != None: 
                cache[self.HPC['encode'][i]] = new_x

        return self.final_attention(x, new_x)



class non_shared_HPC_layer(nn.Module):
...
class non_shared_Hierarchical_Perceiver(nn.Module):
    '''this model specifies a hierarchical perceiver that does not make use of weight sharing in each of the groups'''
    def __init__(self, dim_in, c):
        super().__init__()
        if c.model_type == 'h-npt-large':
        ...
        for index, layer in enumerate(self.HPC['encode']):
            ...
            self.layers.append(HPC_layer(layer_group, self.HPC['latent_vectors_per_group'][index], self.HPC['latent_channels'][index]))    

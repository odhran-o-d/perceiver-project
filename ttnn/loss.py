...
class Loss:
    ...
    * Loss.finalize_epoch_losses() to obtain the loss_dict for the current
    ...
    """
    def __init__(
            self, c, model_uncertainties, metadata, is_minibatch_sgd,
            device=None, tradeoff_annealer=None):
        """

        :param c:
        :param model_uncertainties:
        :param metadata:
    ...
    def compute(self, *args, **kwargs):
        ...
        # self.update_losses(loss_dict, kwargs['eval_model'])
    ...
    def compute_loss(
        ...
        Args:
            ...
            supcon_loss: Optional[torch.Tensor], loss computed by the
                Supervised Contrastive Loss module. This is already computed
                in the TTNN forward method, and here is added to the loss
                metadata.
        ...
        If we use self.model_uncertainties, then MSE and cross-entropy are
        weighted by uncertainties and may be interpreted as likelihoods.
        ...
        # No targets, no target loss. Only a necessary check in imputation
        # (we want to avoid this CUDA synchronization point if possible).
        # if label_mask_matrix.sum() == 0:
        if self.c.is_imputation and dataset_mode == 'train':
            loss_indices['label'] = None
        ...
        elif label_mask_matrix is None:
        ...
        if (not self.c.is_imputation and
                self.c.model_augmentation_bert_mask_prob[dataset_mode] == 0):
        ...
        for col, (out, dat) in enumerate(zip(output, ground_truth_data)):
            ...
            if not is_cat and self.c.data_set not in ['cifar10', 'cifar100']:
            ...
            for mode, mode_loss_indices in loss_indices.items():
                ...
                # (Note that these are not exclusive. In imputation datasets
                # both target and feature values can inhabit the same column.)
        ...
        if supcon_loss is not None:
            loss_dict['supcon'] = {}
            loss_dict['supcon']['cat_loss'] = supcon_loss
    ...
    def finalize_losses(self, raw_dict, eval_model):
        ...
        * if we are evaluating model, also compute losses and accuracies for
        ...
        for mode in raw_dict.keys():
            ...
            if mode == 'supcon':
                std_dict[mode]['total_loss'] = raw_dict[mode]['cat_loss']
                std_dict[mode]['cat_loss'] = raw_dict[mode]['cat_loss']
                continue
    ...
    def balance_self_supervision(self, raw_dict):
        ...
        """
        # TODO: attempt to learn tradeoff? Appears like it will be helpful
        #   for supervised prediction with high percentage missing values

        tradeoff_setting = self.c.exp_tradeoff

        if not self.c.model_supcon_regularizer:
            ...
            if tradeoff_setting == -1:
                ...
                # There may be no losses to balance if we don't do
                #   augmentation or if we don't have a target col
                #   (imputation in training time)
                ...
                if self.dataset_mode == 'train' and self.c.is_imputation:
                    label_loss = 0
                else:
                    label_loss = (
                        (raw_dict['label']['cat_loss'] +
            ...
            if tradeoff_setting == -1:
                ...
                if 'supcon' in raw_dict.keys():
                    supcon_loss = raw_dict['supcon']['cat_loss']
                else:
                    supcon_loss = 0

                tradeoff = self.tradeoff_annealer.curr_tradeoff

                total_loss = tradeoff * supcon_loss + (1 - tradeoff) * label_loss
                # print(supcon_loss)
                # print(total_loss)
                return total_loss
                # return supcon_loss
    ...
    def compute_column_loss(
        ...
        Args:
            ...
            num_preds (torch.Tensor): Number of entries for which we are
                making a prediction. Needed for normalization in multitask
                uncertainty for numerical columns.
        ...
        if is_cat:
            ...
            # Add temperature scaling is as simple as doing.
            # See TTNN.models.multitask_uncertainty
            if self.model_uncertainties:
                output = output / self.model_uncertainties.temp(col)
            ...
            # See TTNN.models.multitask_uncertainty
            if self.model_uncertainties:
                # Each loss term gets multiplied by 1/(2*std^2).
                # The fact that they are already summed here does not
                # change anything.
                loss *= self.model_uncertainties.factor(col)
                # For each loss term we add a single log(std)
                # Since losses are already summed, we account for
                # the number of elements here.
                # Our loss is average negative log likelihood
                # (average over individual predictions)
                loss += num_preds * (
                    self.model_uncertainties.log_stds_with_penalty[col])

            if eval_model and self.c.data_set not in ['cifar10', 'cifar100']:
                ...
                if self.model_uncertainties:
                    # record unnormalised avg log-likelihood value as well
                    log_lik_unstd = loss + num_preds * np.log(sigma)
                    extra_out[self.extras[1]] = log_lik_unstd
                else:
    ...
    # # Make sure we are computing loss over all intended elements
    # # if c.exp_smoke_test:
    # for mode in loss_indices.keys():
    #     mode_loss_indices = loss_indices[mode]
    #     if mode_loss_indices is not None:
    #         num_mode_loss_indices = mode_loss_indices.shape[0]
    #     else:
    #         num_mode_loss_indices = 0
    #
    #     assert (
    #         loss_dict[mode]['num_total_preds'] +
    #         loss_dict[mode]['cat_total_preds']) == num_mode_loss_indices

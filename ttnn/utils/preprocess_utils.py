...
# Deprecated -- we use multiple target cols (cat_target_cols, num_target_cols)
# now (if we update, keep this in mind!)

# def rows_to_matrix_entries(indices, target_col):
#     """Convert list of row indices to 2D matrix entries.
#
#     Entries will be associated with a fixed target_col.
#     """
#     return np.stack([indices, len(indices) * [target_col]], -1)

...
def encode_data(
    ...
    :return: Tuple[encoded_data, input_feature_dims]
            ...
            input_feature_dims: List[int], Size of encoding for each feature.
                Needed to initialise embedding weights in TTNN.
    """
    ...
    if time_code:
        time_sets = [[x[i] for x in time_code[1:]] for i in range(len(time_code[1]))]
        time_set_dic = {}
        for s in time_sets:
            for k in s:
                time_set_dic[k] = s
    ...
    for col_index in range(D):
        ...
        # fit to complete time series if time-series data
        t_cols = None
        if time_code and col_index in time_set_dic.keys():
            t_cols = time_set_dic[col_index]
            t_stat_cols = []
            t_non_missing_cols = []
            for t_col_index in t_cols:
                t_stat_filter = compute_statistics_matrix[:, t_col_index]
                t_stat_cols.append(
                    data_table[t_stat_filter, t_col_index].reshape(-1, 1))

                # Non-missing entries, which we transform
                t_non_missing_filter = non_missing_matrix[:, t_col_index]
                t_non_missing_cols.append(data_table[
                    t_non_missing_filter, t_col_index].reshape(-1, 1))
            t_stat_cols = np.vstack(t_stat_cols)
            t_non_missing_cols = np.vstack(t_non_missing_cols)
        ...
        if col_index in cat_features:
            ...
            if t_cols:
                fit_cat_col = t_non_missing_cols
            else:
                fit_cat_col = non_missing_col
            ...
            if t_cols:
                fit_stat_col = t_stat_cols
            else:
                fit_stat_col = stat_col

            fitted_encoder = StandardScaler().fit(fit_stat_col)

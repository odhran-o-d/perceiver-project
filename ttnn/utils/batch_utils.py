...
def collate_with_pre_batching(batch):
    ...
    We avoid adding a batch dimension, as for TTNN we have pre-batched data,
    ...
    if isinstance(elem, container_abcs.Mapping):
        return elem  # Just return the dict, as there will only be one in TTNN
...
#
#     # Train val test
#     dataset_modes = ([0] * 700) + ([1] * 100) + ([2] * 200)
#     label_col = np.random.choice([0, 1, 2, 3], size=1000, replace=True)
#
#     # Train val
#     # dataset_modes = ([0] * 700) + ([1] * 100)
#     # label_col = np.random.choice([0, 1, 2, 3], size=800, replace=True)
#
#     n_rows = len(dataset_modes)
#     bs = 45
#     n_splits = int(np.ceil(n_rows / bs))
#     train_indices = np.arange(700)
#     stratified_sampler = StratifiedIndexSampler(
#         y=dataset_modes, n_splits=n_splits, shuffle=True,
#         label_col=label_col, train_indices=train_indices)
#
#     batch_order_sorted_row_indices, batch_sizes = (
#         stratified_sampler.get_train_label_stratified_test_array(
#             np.arange(1000)))
#     assert len(set(batch_order_sorted_row_indices)) == len(batch_order_sorted_row_indices)
#     index = 0
#
#     dataset_modes = np.array(dataset_modes)
#
#     from collections import Counter
#
#     for batch_size in batch_sizes:
#         batch_indices = batch_order_sorted_row_indices[index:index+batch_size]
#         index += batch_size
#         print(
#             'dataset mode split in batch: ',
#             Counter(dataset_modes[batch_indices]))
#         train_batch_indices = [index for index in batch_indices if dataset_modes[index] == 0]
#         print(
#             'class split among train rows in the batch',
#             Counter(label_col[train_batch_indices]))

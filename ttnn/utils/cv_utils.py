...
def get_imputation_train_val_test_splits(N, c, exp_run_idx):
    train_val_indices, test_indices = train_test_split(
        np.arange(N), test_size=c.exp_test_perc, shuffle=True,
        random_state=c.np_seed + exp_run_idx)
...
def get_n_cv_splits(c):
    """
    TODO: adapt based on how many splits are provided
    in imputation
    """
    if c.is_imputation:
        return 1

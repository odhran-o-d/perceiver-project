...
from ttnn.batch_dataset import TTNNBatchDataset
from ttnn.datasets.base import BaseDataset
from ttnn.datasets.boston_housing import BostonHousingDataset
from ttnn.datasets.breast_cancer import (
    BreastCancerClassificationDataset, BreastCancerImputationDataset,
    BreastCancerDebugClassificationDataset)
from ttnn.datasets.concrete import ConcreteDataset
from ttnn.datasets.congressional_voting import CongressionalVotingDataset
from ttnn.datasets.connect_4 import Connect4Dataset
from ttnn.datasets.criteo import CriteoDataset
from ttnn.datasets.energy_efficiency import EnergyEfficiencyDataset
from ttnn.datasets.epsilon import EpsilonDataset
from ttnn.datasets.forest_cover import (
    ForestCoverClassificationDataset, ForestCoverImputationDataset)
from ttnn.datasets.higgs import (
    HiggsClassificationDataset, HiggsImputationDataset)
from ttnn.datasets.income import IncomeDataset
from ttnn.datasets.mnist import MNISTDataset
from ttnn.datasets.poker_hand import PokerHandDataset
from ttnn.datasets.protein import ProteinDataset, ProteinImputationDataset
from ttnn.datasets.sarcos import SarcosDataset
from ttnn.datasets.yacht import YachtDataset
from ttnn.datasets.debug import DebugDataset
from ttnn.datasets.ogbn_arxiv import OGBNArxivDataset
from ttnn.datasets.cifar10 import CIFAR10Dataset
from ttnn.datasets.cifar100 import CIFAR100Dataset
from ttnn.datasets.image_utils import load_image_dataloaders
from ttnn.datasets.cora import CoraDataset
from ttnn.datasets.citeseer import CiteSeerDataset
from ttnn.datasets.pubmed import PubMedDataset
from ttnn.datasets.amazon import AmazonDataset
from ttnn.datasets.kick import KickDataset
from ttnn.datasets.IndependentLabelDataset import IndependentLabelDataset
from ttnn.datasets.cluster_dataset import ClusterDataset
from ttnn.datasets.gmm import GMMDataset
from ttnn.datasets.ecoli import EcoliDataset
from ttnn.datasets.svhn import SVHNDataset
from ttnn.datasets.FIDDLE import ARF4heICUDataset, ARF4hMIMICDataset

from ttnn.utils.cv_utils import (
    get_class_reg_train_val_test_splits, get_imputation_train_val_test_splits,
    get_n_cv_splits)
from ttnn.utils.encode_utils import encode_data_dict
from ttnn.utils.memory_utils import get_size
from ttnn.utils.preprocess_utils import (
    get_matrix_from_rows, indices_to_matrix_entries, entries_to_dense)

DATASET_NAME_TO_DATASET_MAP = {
    'eicu': ARF4heICUDataset,
    'mimic': ARF4hMIMICDataset,
    'breast-cancer': BreastCancerClassificationDataset,
    'connect-4': Connect4Dataset,
    'congressional-voting': CongressionalVotingDataset,
    'poker-hand': PokerHandDataset,
    'breast-cancer-imputation': BreastCancerImputationDataset,
    'higgs': HiggsClassificationDataset,
    'higgs-imputation': HiggsImputationDataset,
    'epsilon': EpsilonDataset,
    'criteo': CriteoDataset,
    'forest-cover': ForestCoverClassificationDataset,
    'forest-cover-imputation': ForestCoverImputationDataset,
    'boston-housing': BostonHousingDataset,
    'mnist': MNISTDataset,
    'yacht': YachtDataset,
    'concrete': ConcreteDataset,
    'energy-efficiency': EnergyEfficiencyDataset,
    'income': IncomeDataset,
    'sarcos': SarcosDataset,
    'protein': ProteinDataset,
    'protein-imputation': ProteinImputationDataset,
    ...
    'kick': KickDataset,
    'independentlabel' : IndependentLabelDataset,
...
METADATA_FIELDS = [
    'N', 'D', 'is_imputation', 'cat_features', 'num_features',
    'cat_target_cols', 'num_target_cols', 'input_feature_dims',
    'fixed_test_set_index', 'auroc_setting', 'time_code']
...
class ColumnEncodingDataset:
    ...
    batching strategy for TTNN model.
    """
    def __init__(self, c, device=None):
        ...
        (self.is_row_independent_inference, self.is_torch_model,
            self.row_att) = self.get_model_details(self.c)
        self.mode = None
        ...
        self.c.is_imputation = self._dataset.is_imputation
        self.is_imputation = self._dataset.is_imputation
    ...
    def get_model_details(self, c):
        """
        :param c:
        :return: Tuple[is_row_independent_inference, is_torch_model]
        """
        # Determine Row Interactions

        model_row_independent = False

        # If we use IMAB, we are row-independent.
        if c.model_att_block == 'IMAB':
            model_row_independent = True

        # If we use IMAB for rows in nested TTNN, we are row-independent
        # regardless of model_att_block (which corresponds to the
        # AttentionBlock for the columns)
        if (c.has_column_attention and
                c.model_nested_row_att_block == 'IMAB'):
            model_row_independent = True

        # In all other cases, our model should have row interactions.

        # Determine if model is expecting torch tensors
        is_torch_model = c.model_class in ['TTNN', 'DKL']
        # TODO: alter with more baselines

        # Determine attention being applied to rows
        if c.has_column_attention:
            row_att = c.model_nested_row_att_block
        else:
            row_att = c.model_att_block

        if model_row_independent is True:
            raise NotImplementedError

        return model_row_independent, is_torch_model, row_att
    ...
    def init_cache_path_and_splits(self):
        ...
        if self.c.model_checkpoint_key is not None:
            ...
            model_cache_path = os.path.join(cache_path, self.row_att)
    ...
    def load_datasets(self):
        for cv_split in range(min(self.n_cv_splits, self.c.exp_n_runs)):
            ...
            # For debug -- log dtype of each np.array (and column in datatable)
    ...
    def load_torch_dataset(self, data_dict):
        ...
        if not (self.c.is_imputation and self.c.model_is_semi_supervised):
            mask_torch_data[
                'new_train_val_test_indices'] = data_dict[
                'new_train_val_test_indices']

        return TTNNBatchDataset(
            data_dict=mask_torch_data,
            c=self.c,
            is_row_independent_inference=self.is_row_independent_inference,
    ...
    def get_data_gen_fn(self):
        if self.c.is_imputation:
            dataset_gen_fn = self.generate_imputation_dataset
        else:
            dataset_gen_fn = self.generate_classification_regression_dataset

        return dataset_gen_fn
    ...
    def run_preprocessing_and_caching(self):
        ...
        if not self.are_datasets_cached():
            ...
            # Get dataset generator fn (different for SSL, imputation)
            dataset_gen_fn = self.get_data_gen_fn()
            self.dataset_gen = dataset_gen_fn(data_dict)
    ...
    def generate_imputation_dataset(self, data_dict):
        """
        TODO: docstring

        TODO: handle fixed test split for imputation (will this happen?)
        """
        c = self.c
        data_table = data_dict['data_table']
        missing_matrix = data_dict['missing_matrix']
        ...
        num_target_cols = data_dict['num_target_cols']
        fixed_test_set_index = data_dict['fixed_test_set_index']
        # TODO: add fixed test set logic to imputation, if desired
        # TODO: add fixed train/val/test indices logic (e.g. from Open
        #   Graph Benchmark), if desired

        # Sample random entries from matrix for validation and test
        n_val = int(N * D * c.exp_val_perc)
        n_test = int(N * D * c.exp_test_perc)
        n_take = n_val + n_test
        n_total = N * D

        # Define to cover semi-supervised case, but unused
        train_val_test_rows = None
        new_train_val_test_rows = None
        row_boundaries = None

        for exp_run_idx in range(c.exp_n_runs):
            if c.model_is_semi_supervised:
                # Sample completely at random from matrix.
                all_indices = np.arange(n_total)
                val_test_mask_indices = np.random.choice(
                    all_indices, size=n_take, replace=False)
                val_test_mask_entries = indices_to_matrix_entries(
                    val_test_mask_indices, n_cols=D)
                val_mask_entries = val_test_mask_entries[:n_val]
                test_mask_entries = val_test_mask_entries[n_val:]
            else:
                # In production setting, rows between val and test may not overlap.
                # Therefore, let's split the matrix first into three parts and then
                # do index sampling on that.
                train_val_test_rows = (
                    get_imputation_train_val_test_splits(
                        N=N, c=c, exp_run_idx=exp_run_idx))
                ...
                # are equivariant wrt. rows.
                data_table = np.concatenate([
                    data_table[train_val_test_rows[0]],
                    data_table[train_val_test_rows[1]],
                    data_table[train_val_test_rows[2]]], axis=0)

                lens = np.cumsum([0] + [len(i) for i in train_val_test_rows])
                new_train_val_test_rows = [
                    np.array(list(range(lens[i], lens[i + 1])))
                    for i in range(len(lens) - 1)]

                # Need to rebuild missing matrix with new index ordering
                missing_matrix = missing_matrix[
                    np.concatenate(
                        [indices for indices in new_train_val_test_rows])]

                row_boundaries = {
                    'train': lens[1], 'val': lens[2], 'test': lens[3]}

                # Sample the imputation entries from the val/test rows
                val_indices = np.arange(
                    row_boundaries['train'] * D,
                    row_boundaries['val'] * D)
                val_mask_indices = np.random.choice(
                    val_indices,
                    size=int(
                        len(val_indices) * c.exp_val_perc),
                    replace=False)
                val_mask_entries = indices_to_matrix_entries(
                    val_mask_indices, n_cols=D)

                test_indices = np.arange(
                    row_boundaries['val'] * D,
                    row_boundaries['test'] * D)
                test_mask_indices = np.random.choice(
                    test_indices,
                    size=int(
                        len(test_indices) * c.exp_test_perc),
                    replace=False)
                test_mask_entries = indices_to_matrix_entries(
                    test_mask_indices, n_cols=D)

            # Define Train/Val/Test matrices

            # Train matrix is empty for imputation,
            # because Bert style takes the role of a dedicated training set.
            train_mask_matrix = np.zeros((N, D), dtype=np.bool_)
            val_mask_matrix = entries_to_dense(val_mask_entries, N, D)
            test_mask_matrix = entries_to_dense(test_mask_entries, N, D)

            # TODO: if our missing matrix is (very) large, the proportion of
            # TODO: val and test matrices may vary significantly across epochs
            val_mask_matrix = (val_mask_matrix & ~missing_matrix)
            test_mask_matrix = (test_mask_matrix & ~missing_matrix)

            # Bert Mask Matrix -- all entries eligible for Bert-style-masking
            bert_mask_matrix = ~(
                train_mask_matrix | val_mask_matrix |
                test_mask_matrix | missing_matrix)
            ...
            assert np.array_equal(
                train_mask_matrix ^ val_mask_matrix ^ test_mask_matrix ^
                missing_matrix ^ bert_mask_matrix, np.ones((N, D)))
            assert np.array_equal(
                train_mask_matrix, np.zeros((N, D)))
            assert not np.array_equal(
                val_mask_matrix, np.zeros((N, D)))
            ...
            yield dict(
                ...
                val_mask_matrix=val_mask_matrix,
                test_mask_matrix=test_mask_matrix,
                # bert_mask_matrix=bert_mask_matrix,  # We don't actually need this now -- it's computed later.
                original_dataset_train_val_test_indices=train_val_test_rows,
                new_train_val_test_indices=new_train_val_test_rows,
                row_boundaries=row_boundaries,
                fixed_test_set_index=fixed_test_set_index)


class TTNNDataset(torch.utils.data.Dataset):
    """

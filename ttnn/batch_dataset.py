...
from ttnn.mask import mask_data_for_dataset_mode
from ttnn.utils.batch_utils import StratifiedIndexSampler
from ttnn.utils.cv_utils import DATASET_ENUM_TO_MODE
...
"""
Key format:
(is_semi_supervised, is_imputation, is_row_independent_inference, dataset_mode)
is_semi_supervised: bool
is_imputation: bool
is_row_independent_inference: bool, model has row interactions at test time
    (e.g. SAB does, IMAB does not)
...
BATCHING_SETTINGS_MAP = {
    # SSL Imputation: no easy way to stratify over train, val, test #
    # SSL Imputation, Row Independent Inference = True
    (True, True, True, 'train'): ([0, 1, 2], False),
    (True, True, True, 'val'): ([1], False),  # select all rows with val idxs
    (True, True, True, 'test'): ([2], False),  # select all rows with test idxs
    # SSL Imputation, Row Independent Inference = False
    (True, True, False, 'train'): ([0, 1, 2], False),
    (True, True, False, 'val'): ([0, 1, 2], False),
    (True, True, False, 'test'): ([0, 1, 2], False),

    # SSL Class/Reg: makes sense to stratify train vs val vs test if poss #
    # SSL Class/Reg, Row Independent Inference = True
    (True, False, True, 'train'): ([0, 1, 2], True),
    (True, False, True, 'val'): ([1], False),
    (True, False, True, 'test'): ([2], False),
    # SSL Class/Reg, Row Independent Inference = False
    (True, False, False, 'train'): ([0, 1, 2], True),
    (True, False, False, 'val'): ([0, 1, 2], True),
    (True, False, False, 'test'): ([0, 1, 2], True),

    # Prod Imputation: stratify, assuming train, val, test #
    #            come from non-overlapping rows            #
    # Prod Imputation, Row Independent Inference = True
    (False, True, True, 'train'): ([0], False),
    (False, True, True, 'val'):  ([1], False),
    (False, True, True, 'test'): ([2], False),
    # Prod Imputation, Row Independent Inference = False
    (False, True, False, 'train'): ([0], False),
    (False, True, False, 'val'): ([0, 1], True),
    (False, True, False, 'test'): ([0, 1, 2], True),

    # Prod Class/Reg: makes sense to stratify train vs val vs test if poss #
    # Prod Class/Reg, Row Independent Inference = True
    (False, False, True, 'train'): ([0], False),
    (False, False, True, 'val'): ([1], False),
    (False, False, True, 'test'): ([2], False),
    # Prod Class/Reg, Row Independent Inference = False
    (False, False, False, 'train'): ([0], False),
    (False, False, False, 'val'): ([0, 1], True),
    (False, False, False, 'test'): ([0, 1, 2], True)
}


class TTNNBatchDataset(torch.utils.data.IterableDataset):
    def __init__(
            self, data_dict, c, is_row_independent_inference, curr_cv_split,
            metadata, device, sigmas):
        self.c = c
        self.is_row_independent_inference = is_row_independent_inference
        ...
        if self.c.data_set in ['cifar10', 'cifar100']:
        ...
        # Each key: value pair has format
        # dataset_mode: (mode_row_indices, mode_balance_arr)
        #   * mode_row_indices are the row indices from the base data used in
        #       this particular mode.
        #   * mode_balance_arr is provided in non-SSL imputation cases when we
        #       aim to stratify batches across train/val/test
        ...
        # Based on the setting determined by
        #   is_semi_supervised, is_imputation, is_row_independent_inference
    ...
    def prepare_image_dataset(self, data_dict):
        if self.c.data_set not in ['cifar10', 'cifar100']:
            raise NotImplementedError

        if self.c.data_set == 'cifar10':
            self.num_classes = 10
        elif self.c.data_set == 'cifar100':
    ...
    def load_image_dataset(self, epoch):
        # Data will already be RandAugmented and encoded
        ...
        if self.c.model_image_validity_test:
            sums = [torch.zeros(1), torch.zeros(1), torch.zeros(1)]
        ...
        for dataset_mode, loader in enumerate(loaders):
            for image_batch, label_batch in loader:
                ...
                if self.c.model_image_validity_test:
                    sums[dataset_mode] += torch.sum(image_batch)
    ...
    def get_batch_indices(self, dataset_mode):
        """
        Key assumption: In production imputation,
        train, val, test need to come from non-overlapping rows.

        Batch indices are determined by three design settings:
        Yes/No SSL
        Yes/No Imputation
        Yes/No Model has row interactions at test time
            (e.g. SAB does, IMAB does not)
        :return: Tuple[n_rows, batch_modes, mode_indices, stratified_sampler]
            n_rows: int, the number of rows used in this mode setting. e.g. for
                SSL, will be all of the rows available (unless we are at val/
                test time, and are using IMAB, in which inference on a
                particular row is independent from other rows).
            ...
            stratified_sampler: if we should mode balance the batches, this
                ...
                Done in all cases but SSL imputation, if the user has specified
                c.exp_batch_mode_balancing.
        """
        batch_modes, mode_balance = BATCHING_SETTINGS_MAP[(
            self.c.model_is_semi_supervised,
            self.c.is_imputation,
            self.is_row_independent_inference,
            dataset_mode
        )]
        ...
        # In all cases other than SSL Imputation, we can simply add the
        # train_val_test_indices that are specified in the batch_modes
        # (this tells us the batching logic).
        if not (self.c.is_imputation and self.c.model_is_semi_supervised):
            ...
            # In SSL Imputation, if we take as input {train, val, test},
            # just provide a full train index list.
            # This is acceptable because we will not attempt to stratify
            # over modes in SSL Imputation batching.
            if batch_modes == [0, 1, 2]:
                mode_indices.append(np.arange(self.metadata['N']))
            else:
                # for given data mode select all rows with any idxs
                if batch_modes == [1]:  # val mode
                    chosen_matrix = self.data_dict['val_mask_matrix']
                    mode_indices.append(np.where(chosen_matrix.sum(axis=1))[0])

                elif batch_modes == [2]:  # test mode
                    chosen_matrix = self.data_dict['test_mask_matrix']
                    mode_indices.append(np.where(chosen_matrix.sum(axis=1))[0])
                else:
                    raise Exception(
                        'Should never be solely providing train '
                        'matrix as model input in SSL Imputation.')
    ...
    def batch_gen(self):
        ...
        # Hack to avoid stratifying when we have a super small batch size
        # TODO @nband Should ideally check against number of classes
        #   (in classification)

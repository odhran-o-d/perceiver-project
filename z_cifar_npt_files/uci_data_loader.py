...
class UCIDatasets():
    def __init__(self,  name,  data_path="data", n_splits = 10):
        self.datasets = {
    ...
    def _load_dataset(self):
        ...
        if self.name == "housing":
            ...
            data = pd.read_csv(self.data_path+'UCI/wdbc.data', header=None)
            cols = data.columns.tolist()
            cols = cols[1:]
            cols = cols[1:] + cols[:1]
            data = data[cols]
            data[data.columns[-1]] = pd.Categorical(data[data.columns[-1]])
    ...
    def get_split(self, split=-1, train=True):
        ...
        if 0<=split and split<self.n_splits: 
            ...
            if self.name != 'breast' and self.name != 'wine':
